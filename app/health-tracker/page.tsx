...
<div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">{healthMetrics.map((metric) => { const Icon = metric.icon; return (<div key={metric.id} onClick={() => setActiveMetric(metric.id)} className={`${theme.cardBg} rounded-2xl p-6 shadow-xl ${theme.border} border transform hover:scale-105 transition-all duration-300 cursor-pointer group ${activeMetric === metric.id ? 'ring-4 ring-blue-500 ring-opacity-50' : ''}`}>
<div className="flex items-start justify-between mb-4"><div className={`w-12 h-12 rounded-xl bg-gradient-to-br ${metric.color} flex items-center justify-center transform group-hover:rotate-6 transition-transform duration-300`}><Icon className="w-6 h-6 text-white"/></div><span className={`px-3 py-1 rounded-full text-xs font-semibold ${metric.status === 'Normal' || metric.status === 'Good' || metric.status === 'Stable' || metric.status === 'Active' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200'}`}>{metric.status}</span></div><h3 className={`text-sm ${theme.textSecondary} mb-2`}>{metric.label}</h3><div className="flex items-baseline space-x-2"><span className="text-3xl font-bold">{metric.value}</span><span className={`text-sm ${theme.textSecondary}`}>{metric.unit}</span></div></div> )})}</div>
